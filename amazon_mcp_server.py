...
A Model Context Protocol server that provides tools for accessing Amazon SP API
"""
...
# Amazon SP API client (you'll need to install: pip install python-amazon-sp-api)
    ...
    async def get_product_info(self, sku: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = catalog_api.get_listings_item(
                sellerId=seller_id, sku=sku, marketplace_id=self.marketplace.value[1], includedData=['attributes', 'summaries', 'issues', 'offers', 'fulfillmentAvailability', 'procurement', 'relationships', 'productTypes' ]
    )
...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[types.TextContent]:
    ...
    user_credentials = None  # In your webapp, you'd load this from database
